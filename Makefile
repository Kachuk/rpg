...
movement_handler=src/engine/movement_handler/movement_handler.c
...
files=deps/src/glad.c src/main.c src/engine/global.c $(render) $(io) $(config) $(input) $(time) $(physics) $(movement_handler) $(array_list) $(entity) $(animation) $(audio)
libs=-lm `sdl2-config --cflags --libs` -lSDL2_mixer `pkg-config --libs glfw3` -ldl

build:
	gcc -g3 -O0 -I./deps/include $(files) $(libs) -o rpg.out
